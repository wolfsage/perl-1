...
S_simplify_sort_aelem(pTHX_ OP **o)
{
	...
	PerlIO_printf(PerlIO_stderr(), "aelem\n");
     ...
     * that it's safe to access the IV more directly. Or can OP_CONST
     * be other stuff? Do we need to check SvIOK or other stuff? */
    /* FIXME is the intermediate store in an SV necessary? Is an
     * intermediate required for the ->op_last? Compiler fu failure
     * on my part. */
    const_sv = cSVOPx_sv(kBINOP->op_last);
    const_val = SvIV(const_sv);
	PerlIO_printf(PerlIO_stderr(), "!!!%i\n", (int)const_val);
    if (const_val != 0)
	return FALSE;
...
S_simplify_sort(pTHX_ OP *o)
{
	...
	PerlIO_printf(PerlIO_stderr(), "alive2\n");
    if (is_aelem) {
	/* array-deref optimization path: $a->[0] <=> $b->[0] or similar */

	kid = kBINOP->op_first;				/* get past cmp */
	/* check if this is an array deref with a constant */
	is_aelem = simplify_sort_aelem(&kid);
	if (!is_aelem)
	    return;
	PerlIO_printf(PerlIO_stderr(), "aelem matched fully left\n");
    }
	...
	PerlIO_printf(PerlIO_stderr(), "aelem2\n");
	/* array-deref optimization path: $a->[0] <=> $b->[0] or similar */

	kid = kBINOP->op_last;				/* get past cmp */
	PerlIO_printf(PerlIO_stderr(), "PTR: %u\n", kid);
	/* check if this is an array deref with a constant */
	is_aelem = simplify_sort_aelem(&kid);
	if (!is_aelem)
	    return;
	PerlIO_printf(PerlIO_stderr(), "PTR: %u\n", kid);
	PerlIO_printf(PerlIO_stderr(), "aelem matched fully right\n");
    }
    else { /* normal optimization path: $a <=> $b or similar */
